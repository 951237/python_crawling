...
# 더보기 클릭
sleep(2)
excute_script("javascript:fnGoodsAttrNmFold('show');")
...
sleep(2)
print('등판재질을 선택합니다.')
excute_script("javascript:attrNmValLink('5611210201', '등판재질',  "
...
for 체크박스 in 체크박스리스트:
    ...
    for i in 필수옵션:  # 필수옵션 리스트 2개 확인
        if i in parent.text and '가죽' not in parent.text:  # '메시'나 '망사'를 포함하지만,
            # '가죽'을 포함하지 않는다면
            체크박스.click()
excute_script("javascript:toSMPPIntgrSrchGoodsList('');")   #조회 버틑 클릭
...
sleep(2)
print('좌판재질을 선택합니다.')
excute_script("javascript:attrNmValLink('5611210201', '좌판재질',  'ATTR_264449' , '' ); ")  # 자바스크립트 코드 실행
...
for 체크박스 in 체크박스리스트:
    ...
    for i in 필수옵션:  # 필수옵션 리스트 2개 확인
        if i in parent.text and '가죽' not in parent.text:  # '메시'나 '망사'를 포함하지만,
            # '가죽'을 포함하지 않는다면
            체크박스.click()
excute_script("javascript:toSMPPIntgrSrchGoodsList('');")   #조회 버 클릭

# 팔걸이 유
sleep(2)
excute_script("javascript:fnGoodsAttrNmFold('show');")
print('팔걸이 유무 선택합니다.')
excute_script("javascript:attrNmValLink('5611210201', '팔걸이유무',  'ATTR_259429' , '' ); ")  # 자바스크립트 코드 실행
...
# 리스트를 100개로 정렬
sleep(2)
print('리스트를 100개로 정렬합니다.')
driver.find_element_by_css_selector('option[value = "100"]').click()
sleep(2)
excute_script("javascript:searchForNewPageSize();")

# 인증검색에 우선구매대상 + 의무구매대상 체크
sleep(2)
print('우선구매대상 + 의무구매대상을 선택합니다.')
...
# 엑셀파일 생성하기
filename = '/Users/mac/Documents/python_work/my_project/crawling/크롤링_나라장터_200202/chair_list.xlsx'
book = Workbook()
book.save(filename)
print(f'{filename} 파일을 생성합니다.')
...
with pd.ExcelWriter(filename, engine = 'openpyxl', mode = 'a') as writer:
    ...
    for idx, script in enumerate(스크립트_리스트):
        print(f'{script}페이지 이동합니다.')
        excute_script(script)
        print(idx)
        sleep(1)
